...
class Account:
    ...
    account_lock : Lock()
        Lock utilizado para proteger a região onde é feito o deposito
    account_semaphore : Semaphore
        Semaforo binário para que apenas seja feita uma retirada por vez
    valor_especial_usado: int
        Valor do utilizado do cheque especial
    ...
    account_lock = Lock()
    account_semaphore = Semaphore(1)
    valor_especial_usado = 0
    ...
    def withdraw(self, amount: int) -> bool:
        ...
        self.account_semaphore.acquire()
        self.valor_especial_usado = 0
        ...
        if self.balance >= amount:
            ...
            if self.overdraft_limit >= overdrafted_amount:
                self.balance -= (amount - overdrafted_amount)
                self.overdraft_limit -= overdrafted_amount
                self.valor_especial_usado = overdrafted_amount
